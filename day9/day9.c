    ...
    while(idx < length)
    {
        ...
        switch(opcode)
        {
...
#endif
                printf("%d\n", modes[0] ? op1 : numbers[op1]);
                idx += 2;
                break;
            case 5: // jump-if-true
                op1 = numbers[idx+1];
                op2 = numbers[idx+2];
                if((modes[0] ? op1 : numbers[op1]) != 0)
                    idx = (modes[1] ? op2 : numbers[op2]);
                else
                    idx += 3;
                break;
            ...
            case 7: // less than
                ...
                printf("op: %d, modes: %d %d %d, op1: %d, op2: %d, op3: %d\n", opcode, modes[0], modes[1], modes[2], op1, op2, op3);
#endif
                numbers[op3] = ((modes[0] ? op1 : numbers[op1] ) < (modes[1] ? op2 : numbers[op2]));
                idx += 4;
                break;
            case 8: // equals
                op1 = numbers[idx+1];
