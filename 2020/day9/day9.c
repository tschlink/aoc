...
void copyArray(long* in, long* out, int nb)
{
    int i = 0;
    while(i < nb)
    {
...
long getParam(int val, long mem[], int mode, int relbase)
{
    if(mode == 0)
    {
        return mem[val];
...
int compute(long input[], int length)
{
    int memsize = 30000;
    long *numbers = calloc(memsize, sizeof(long));
    ...
    while(idx < length)
    {
        ...
        switch(opcode)
        {
            ...
            case 2: // *
#if DEBUG
                printf("op: %d, modes: %d %d %d, op1: %ld, op2: %ld, op3: %ld\n", opcode, modes[0], modes[1], modes[2], op1, op2, op3);
#endif
